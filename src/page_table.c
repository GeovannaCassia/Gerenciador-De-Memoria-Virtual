...
{
    if (page < 0 || page >= PAGE_TABLE_SIZE || frame < 0 || frame >= NUM_FRAMES) {
        return;
    }
...
void page_table_update_aging(void)
{
    for (int i = 0; i < PAGE_TABLE_SIZE; i++) {
        if (page_table[i].valid) {
            /* Deslocar contador para a direita */
            page_table[i].aging_counter >>= 1;

            /* Inserir reference_bit no MSB (bit 7 para 8 bits) */
            if (page_table[i].reference_bit) {
                page_table[i].aging_counter |= 0x80;
            }

            /* Zerar o bit de referência */
            page_table[i].reference_bit = 0;
        }
    }
}
